...
#include <string>
#include <algorithm>
#include "/Users/hugo/Documents/Exercice-5-physnum/common/ConfigFile.h"
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (v_uniform) {
            ...
            if (x[i]<=xa) {
                h0[i] = hL;
            } else if (x[i]>xa && x[i]<=xb) {
                h0[i] = hL + (hR-hL)*pow(sin(PI*(x[i]-xa)/(2*xb-2*xa)),2);
            } else if (x[i]>xb && x[i]<=xc) {
                h0[i] = hR;
            } else if (x[i]>xc && x[i]<=xd) {
                h0[i] = hR - (hR-hL)*pow(sin(PI*(xc-x[i])/(2*xc-2*xd)),2);
            } else {
                h0[i] = hL;
            } // TODO: profil de récif selon la donnée du problème
        }
        vel2[i] = g * h0[i];
    }
    ...
    double dt = CFL*dx/sqrt(max_vel2); // MODIFIER
    if (impose_nsteps) {
        dt  = tfin/nsteps; // MODIFIER, pourque on as 'nsteps' temporelle
        CFL = sqrt(max_vel2)*dt/dx; // MODIFIER
    }
    ...
    for (int i = 0; i < N; ++i) {
        beta2[i] = vel2[i]; // TODO: calculer beta^2 aux points de maillage
        fnow[i]  = 0.0;
        fpast[i] = 0.0; // TODO: Implementer une condition initiale statique
    }
        ...
        for (int i = 1; i < N - 1; ++i) {
            if (equation_type == "A") {
                fnext[i] = 2*fnow[i] - fpast[i]
                    + beta2[i] * (fnow[i+1] - 2*fnow[i] + fnow[i-1]);
            } else if (equation_type == "B") {
                fnext[i] = 2*fnow[i] - fpast[i]
                    + (beta2[i+1] - beta2[i-1]) / 4.0 * (fnow[i+1] - fnow[i-1])
                    + beta2[i] * (fnow[i+1] - 2*fnow[i] + fnow[i-1]);
            } else if (equation_type == "C") {
                fnext[i] = 2*fnow[i] - fpast[i]
                    + beta2[i+1]*fnow[i+1] - 2*beta2[i]*fnow[i] + beta2[i-1]*fnow[i-1];
            }
        }
